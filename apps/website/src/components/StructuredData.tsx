...
const StructuredData = () => {
  ...
  const serviceSchemas: Record<string, object> = Object.fromEntries(
    Object.entries(structuredData.serviceSchemas).map(([path, service]) => [
      path,
      {
        ...
        // `offers` nur ausgeben, wenn ein Angebot hinterlegt ist. Preise werden
        // bewusst nicht pauschal veröffentlicht (auch nicht maschinenlesbar).
        ...(service.offer
          ? {
              "offers": {
                "@type": "Offer",
  ...
  // `websiteSchema` wird hier NICHT mehr injiziert: scripts/prerender-seo.mjs
  // backt seit 2026-08 auf jeder Route einen WebSite-Block mit demselben
  // `@id` (https://newedgebrand.com/#website) fest ins HTML. Injizierten wir
  // ihn zusätzlich, stünden nach dem Hydrieren zwei Knoten mit derselben
  // Kennung im <head> — im Browser nachgemessen — und Google bekäme zwei
  // widersprüchliche Definitionen derselben Entität. Der <Helmet>-Block unten
  // bleibt unangetastet; er rendert diese Schemas ohnehin nicht.
  useEffect(() => {
    // FAQPage der Startseite bewusst NICHT mehr ausgeliefert: Die fünf Fragen
    // aus structuredData.faq werden von keiner Komponente gerendert — sie stehen
    // nirgends sichtbar auf der Seite. Googles Richtlinie für FAQPage verlangt
    // ausdrücklich sichtbare Fragen und Antworten; ausgezeichnete, aber
    // unsichtbare FAQ werden ignoriert und können eine manuelle Maßnahme
    // auslösen. Die 20 FAQPage-Schemas der Blueprint- und Branchenseiten bleiben:
    // die speisen sich aus dem `faq`-Feld derselben Seiten und sind dort sichtbar.
    // Soll das Schema zurück, müssen die Fragen zuerst auf die Startseite.
    const extra = [
