...
const withAccent = (text: string, phrase: string, dark = false) => {
  ...
  return (
    <>
      {text.slice(0, idx)}
      {dark ? (
        <span style={{ color: LIME, fontWeight: 600 }}>{phrase}</span>
      ) : (
...
const KiAudit = () => {
  ...
  /* Seiten-Chrome, das (noch) nicht im Content-Layer liegt. Muss sprach-
     abhängig sein — diese Strings rendern auch auf /en/ki-audit. Werte
     (25 Std., 5–10, ≥ 3) bleiben in beiden Sprachen identisch. */
  const t = {
  ...
  const stats = [
    { value: isEn ? "25 hrs" : "25 Std.", label: isEn ? "Effort for your team" : "Aufwand für euer Team" },
    { value: "5–10", label: isEn ? "Business days to the report" : "Werktage bis zum Report" },
    { value: "≥ 3", label: isEn ? "Guaranteed use cases" : "Garantierte Use Cases" },
  ];
                  ...
                  {kiAudit.fit.passtNicht.map((item, i) => (
                    <li key={i} className="flex items-start gap-3">
                      <span className="shrink-0 w-5 h-5 flex items-center justify-center mt-0.5" style={{ background: "rgba(23,23,23,0.06)", borderRadius: "50%" }}>
                        <IconX size={12} stroke={2.2} style={{ color: "#5E5E5A" }} />
                      </span>
